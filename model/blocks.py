        ...
        for layer in self:
            if isinstance(layer, TimestepBlock):
                if isinstance(emb, tuple):
                    x = layer(x, emb=emb[0], cond=cond, lateral=lateral)
                else:
                    x = layer(x, emb=emb, cond=cond, lateral=lateral)
            elif isinstance(layer,AttentionBlock2):
                _,CrossAttentions, CA_condision=emb
                h_=CrossAttentions(x,CA_condision)
                x = layer(x, h_)
            else:
                x = layer(x)
        return x
...
class AttentionBlock2(nn.Module):
    ...
    def forward(self, x,h_):
        return torch_checkpoint(self._forward, (x,h_ ), self.use_checkpoint)

    def _forward(self, x,h_):
        #h_,alpha=h_
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1)
        qkv = self.qkv(self.norm(x))
...
class CrossAttention2(nn.Module):
    ...
    def __init__(
        ...
        #self.norm_kv = normalization(query_dim)   #condision
        self.k= conv_nd(1, query_dim, hw, 1)
        self.v = conv_nd(1, query_dim, hw, 1)

        self.norm_q = normalization(channels)  # feature
        ...
        if use_new_attention_order:
            ...
            # split heads before split qkv
            self.attention = QKVAttentionLegacy_qkv(self.num_heads)

    def forward(self, x,condision):
        return torch_checkpoint(self._forward, (x, condision), self.use_checkpoint)

    def _forward(self, x,condision):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1)
        q = self.q(self.norm_q(x))

        b, c = condision.shape
        condision = condision.reshape(b, c, -1)
        k = self.k(condision).reshape(b, 1, -1)
        v = self.v(condision).reshape(b, 1, -1)
...
class QKVAttentionLegacy_qkv(nn.Module):
    """
